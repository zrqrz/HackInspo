...
const storyFont = "font-['Helvetica_Neue',Helvetica,Arial,sans-serif] text-base leading-[1.625]";
...
const proseStory = [
  ...
  "prose-p:text-white/70 prose-p:leading-[1.625]",
  "prose-li:text-white/70 prose-li:leading-[1.625]",
        ...
        {items.map((item) => {
          ...
          return (
            <section key={item.id} id={item.id} className="scroll-mt-8">
              {item.title ? (
                sub ? (
                  <h3 className="mt-0 mb-[0.6em] text-lg font-medium leading-tight tracking-normal text-white/90">
                    {item.title}
                  </h3>
                ) : (
                  <h2 className="mt-0 mb-[0.6em] text-[1.5em] font-medium leading-tight tracking-normal text-white/90">
                    {item.title}
                  </h2>
                )
              ) : null}
  ...
  if (plainDescription) {
    ...
    return (
      <div className={storyFont}>
        <p className="m-0 whitespace-pre-wrap text-white/70">{plainDescription}</p>
      </div>
    );
  }

  return <p className="text-white/30 italic">No description available.</p>;
}
